...
/**
 * Ambience is a Broadcast Receiver registered and used by an Android component
 * (Activity,Fragment or Service) to send and receive update playback control
 * status and information to and from the AmbientService.
 *
...
public class Ambience extends BroadcastReceiver {
     ...
     */
    private static final String TAG = "com.radioayah.ambience.Ambience";
    ...
    /**
     * Boolean value used to indicate if Ambience is listening for updates from
     * the AmbientService
     */
    private static boolean isListeningForUpdates = false;

    /**
     * Boolean value used to indicate if the AmbientService is started or has
     * stopped
     */
    ...
    /**
     * Method used to initialize the Ambience. This method starts the
     * AmbientService if it is not alreadyStarted.
     *
    ...
    /**
     * Method used to unregister Ambience with the Broadcast Manager. The
     * Callback Listener will no longer receive updates.
     *
    ...
    /**
     * Method used to check if Ambience is listening for updates from the
     * AmbientService
     *
     * @return A boolean value indicating if Ambience is listening for updates
     * from AmbientService
     */
     ...
     *
     * @return boolean value indicating if the AmbientService has started or
     * stopped
     */
    ...
    private static void stopAmbientService() {
        if (hasAmbientServiceStarted()) {
            ...
            // AmbientMediaBrowserService.class));
    ...
    /**
     * Method used to release all resources held by Ambience. This method
     * un-registers Ambience and stops the AmbientService. This is a destructive
     * method. All media playback, updates and notifications will be cancelled.
     */
     ...
     *
     * @param callback An Android component that will handle all playback control
     *                 callbacks from the AmbientService. The Android component can
     *                 be an Activity, Fragment or Service. Only a single Android
     *                 component can receive updates from the AmbientService.
     * @return An Ambience object
     */
    ...
    public Ambience setPlaylistTo(AmbientTrack[] playlist) {
        ...
        sendPlaylistToAmbientService(new ArrayList<Parcelable>(
        ...
        for (int x = 0; x < playlist.size(); x++) {
            if (playlist.get(x) == null) {
                throw new NullPointerException(TAG
                        + ": AmbientTrack at position" + x + " cannot be null.");
            }
        }
    ...
    public Ambience shufflePlaylist() {
        Intent intent = getAmbientServiceIntentInstance();
        intent.putExtra(AmbientService.SHUFFLE_MODE,
                AmbientService.ShuffleMode.ON);
        sendIntentToAmbientService(intent);
    ...
    public Ambience unShufflePlaylist() {
        Intent intent = getAmbientServiceIntentInstance();
        intent.putExtra(AmbientService.SHUFFLE_MODE,
                AmbientService.ShuffleMode.OFF);
        sendIntentToAmbientService(intent);
    ...
    public Ambience repeatASingleTrack() {
        Intent intent = getAmbientServiceIntentInstance();
        intent.putExtra(AmbientService.REPEAT_MODE,
                AmbientService.RepeatMode.REPEAT_ONE);
        sendIntentToAmbientService(intent);
    ...
    public Ambience repeatAllTracks() {
        Intent intent = getAmbientServiceIntentInstance();
        intent.putExtra(AmbientService.REPEAT_MODE,
                AmbientService.RepeatMode.REPEAT_ALL);
        sendIntentToAmbientService(intent);
    ...
    /**
     * Method used to append a launch activity request to the Ambience
     * notification
     *
     * @param intentFilterAction intent Filter Action Name to launch a specific activity. This
     *                           should be a unique identifier for your activity
    ...
    /**
     * Helper method used to create a new Intent object used to send messages to
     * the AmbientService
     *
    ...
    /**
     * Method used to play a track at a certain position in the playlist.
     *
    ...
    public void play() {
        Intent intent = getAmbientServiceIntentInstance();
        intent.putExtra(AmbientService.PLAYBACK_STATE,
                AmbientService.PlaybackState.PLAY);
        sendIntentToAmbientService(intent);
    }
    ...
    public void stop() {
        Intent intent = getAmbientServiceIntentInstance();
        intent.putExtra(AmbientService.PLAYBACK_STATE,
                AmbientService.PlaybackState.STOP);
        sendIntentToAmbientService(intent);
    }
    ...
    public void resume() {
        Intent intent = getAmbientServiceIntentInstance();
        intent.putExtra(AmbientService.PLAYBACK_STATE,
                AmbientService.PlaybackState.RESUME);
        sendIntentToAmbientService(intent);
    }
    ...
    public void skip() {
        Intent intent = getAmbientServiceIntentInstance();
        intent.putExtra(AmbientService.PLAYBACK_STATE,
                AmbientService.PlaybackState.SKIP);
        sendIntentToAmbientService(intent);
    }
    ...
    public void previous() {
        Intent intent = getAmbientServiceIntentInstance();
        intent.putExtra(AmbientService.PLAYBACK_STATE,
                AmbientService.PlaybackState.PREVIOUS);
        sendIntentToAmbientService(intent);
    }
    ...
    public void pause() {
        Intent intent = getAmbientServiceIntentInstance();
        intent.putExtra(AmbientService.PLAYBACK_STATE,
                AmbientService.PlaybackState.PAUSE);
        sendIntentToAmbientService(intent);
    }
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        try {
            AmbientService.PlaybackState state = (AmbientService.PlaybackState) intent
                    .getSerializableExtra(AmbientService.PLAYBACK_STATE);
            if(state == null)
            {
                return;
            }
            ...
            switch (state) {
                case PLAY:
                    mCallback.ambienceTrackIsPlaying();
                    break;
                case STOP:
                    mCallback.ambienceTrackHasStopped();
                    break;
                case PAUSE:
                    mCallback.ambienceTrackIsPaused();
                    break;
                case PREPPING_TRACK:
                    mCallback.ambienceIsPreppingTrack();
                    break;
                case END_OF_PLAYLIST:
                    mCallback.ambiencePlaylistCompleted();
                    break;
                case CURRENT_PLAYING_TRACK_INFO: {
                    if (intent.hasExtra(AmbientService.TRACK_PROGRESS)) {
                        mCallback.ambienceTrackCurrentProgress(intent.getIntExtra(
                                AmbientService.TRACK_PROGRESS, 0));
                    }

                    if (intent.hasExtra(AmbientService.TRACK_DURATION)) {
                        mCallback.ambienceTrackDuration(intent.getIntExtra(
                                AmbientService.TRACK_DURATION, 0));
                    }

                    if (intent.hasExtra(AmbientService.CURRENT_TRACK)) {
                        mCallback.ambiencePlayingTrack((AmbientTrack) intent
                                .getParcelableExtra(AmbientService.CURRENT_TRACK));
                    }

                    break;
                }
            /*
			 * No specific error message is passed. The callback must assume
			 * that the track could not be played and attempt to retry or get
			 * feedback from the user.
			 */
                case ERROR:
                    mCallback.ambienceErrorOccurred();
                    Toast.makeText(mContext, "Unknown Error Occured.",
    ...
    /**
     * Interface Listener that must be implemented by An Android Component such
     * as an Activity, Fragment or Service to receive updates from Ambience.
     * Some methods in this interface are called more frequently than others.
     */
    public interface AmbientListener {

        /**
         * Method used to update an Android Component when the track is being
         * prep for play.
         */
        void ambienceIsPreppingTrack();
         ...
         */
        void ambienceTrackDuration(int time);

        /**
         * Method used to update an Android Component with the current playing
         * track. This method is called often
         *
         * @param track Current playing track
         */
        void ambiencePlayingTrack(AmbientTrack track);

        /**
         * Method used to update an Android Component with current progress time
         * of the current playing track. This method is called often
         *
         * @param time Current progress of the track
         */
        void ambienceTrackCurrentProgress(int time);

        /**
         * Method used to alert an Android Component when a track begins playing
         */
        void ambienceTrackIsPlaying();

        /**
         * Method used to alert an Android Component when a track is paused
         */
        void ambienceTrackIsPaused();

        /**
         * Method used to alert an Android Component when a track has stopped
         */
        void ambienceTrackHasStopped();

        /**
         * Method used to update an Android Component when the AmbientService
         * has completed playing a playlist.
         */
        void ambiencePlaylistCompleted();

        /**
         * Method used to update an Android Component when an error occurred in
         * the AmbientService. No specific error message is passed. The Android
         * Component must assume that the track could not be played. Use this
         * method to retry playing the track or take another action.
         *
         * @see "Logcat for the specific error message from the AmbientService"
         */
        void ambienceErrorOccurred();
         ...
         */
        void ambienceServiceStarted(Ambience activeInstance);

        /**
         * Called when the AmbientService is stopped and is no longer listening
         * for requests
         *
         * @param activeInstance The active The active instance of ambience
         */
        void ambienceServiceStopped(Ambience activeInstance);
    }
}
