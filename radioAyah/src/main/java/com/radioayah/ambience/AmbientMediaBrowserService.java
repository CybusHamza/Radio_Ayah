package com.radioayah.ambience;
...
/**
 * AmbientTvService is a specific Android Service for androidTV and androidAuto
 * that is used to control media playback for audio. AmbientService is advanced
 * and has all basic audio control types like
 * play,pause,stop,resume,previous,skip, shuffle and repeat. The Service also
 * creates A notification in the Notification Drawer to control playback. Use
 * the Ambience Class to communicate with the AmbientService
 *
 * @author TonyoStudios.com. Created on 11/18/2014. Updated on 12/01/2014.
 * @version 1.3
 */
@TargetApi(Build.VERSION_CODES.LOLLIPOP)
public class AmbientMediaBrowserService extends MediaBrowserService implements
     ...
     */
    public final static String MEDIA_SESSION_TOKEN_TAG = AmbientService.TAG
            + ".MEDIA_SESSION_TOKEN_TAG";
    ...
    /**
     * ArrayList used to manage AmbientTracks sent to the AmbientService for
     * processing
     */
    ...
    /**
     * Runnable object used to update the main user interface with playback
     * values
     */
    private final Runnable mUpdateProgress = new Runnable() {
        ...
        public void run() {
            ...
            if (mHandler != null && mPlayer != null && mPlayer.isPlaying()) {
                ...
                bundle.putSerializable(AmbientService.PLAYBACK_STATE,
                        AmbientService.PlaybackState.CURRENT_PLAYING_TRACK_INFO);

                bundle.putInt(AmbientService.TRACK_PROGRESS, position);
                bundle.putParcelable(AmbientService.CURRENT_TRACK,
                        mAmbientTrack);
                ...
                mHandler.postDelayed(this,
                        AmbientService.AUDIO_PROGRESS_UPDATE_TIME);
            }
        }
    };
    /**
    ...
    /**
     * Holds the intent filter action name used to launch an activity when a now
     * playing card is clicked on.
     */
    ...
    /**
     * The IncomingRequestBroadcast Receiver responds to all request made to the
     * Ambient Service. It is used to traffic all specific request to their
     * intended route.
     */
    private final BroadcastReceiver IncomingRequestReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (mBundle.containsKey(AmbientService.ACTIVITY_LAUNCHER)) {
                mActivityLauncher = mBundle
                        .getString(AmbientService.ACTIVITY_LAUNCHER);
            ...
            if (mBundle.containsKey(AmbientService.PLAYBACK_STATE)) {
                ...
                try {
                    AmbientService.PlaybackState state = (AmbientService.PlaybackState) mBundle
                            .getSerializable(AmbientService.PLAYBACK_STATE);
                    ...
                    switch (state) {
                        ...
                        case PREVIOUS:
                            ...
                            break;
                        default:
                            throw new IllegalStateException(AmbientService.TAG
     ...
     * unique integer token representing the start request. The Ambient Service
    ...
    public void onCreate() {
        ...
        IntentFilter filter = new IntentFilter(
                AmbientService.AMBIENT_SERVICE_BROADCASTER);
    ...
    /**
     * Called to create a set a media session object for AndroidTV Now Playing
     * Card
     */
     ...
     * playlist, shuffle mode, repeat mode, volume level and action
     * launcher string
     */
    private Bundle buildAmbientServiceBundle() {
        ...
        if (mActivityLauncher != null) {
            bundle.putString(AmbientService.ACTIVITY_LAUNCHER,
                    mActivityLauncher);
        }
    ...
    private void createPlaylist() {
        if (mBundle == null
                || !mBundle.containsKey(AmbientService.PLAYLIST)
                || mBundle.getParcelableArrayList(AmbientService.PLAYLIST) == null) {
            throw new IllegalStateException(AmbientService.TAG
                    + ": needs at least one AmbientTrack item to play");
        }
        ...
        ArrayList<Parcelable> newTracks = mBundle
                .getParcelableArrayList(AmbientService.PLAYLIST);
    ...
    private void removeTrackFromPlaylist() {
        if (mBundle.getParcelable(AmbientService.REMOVE_TRACK) != null
                && mOriginalPlaylist != null && mPlaylist != null) {
            mOriginalPlaylist.remove(mBundle
                    .getParcelable(AmbientService.REMOVE_TRACK));
            mPlaylist
                    .remove(mBundle.getParcelable(AmbientService.REMOVE_TRACK));
        }
    }
    ...
    private void addTrackToPlaylist() {
        if (mBundle.getParcelable(AmbientService.ADD_TRACK) != null
                && mOriginalPlaylist != null && mPlaylist != null) {
            mOriginalPlaylist.add((AmbientTrack) mBundle
                    .getParcelable(AmbientService.ADD_TRACK));
            mPlaylist.add((AmbientTrack) mBundle
                    .getParcelable(AmbientService.ADD_TRACK));

            if (mShuffleState == AmbientService.ShuffleMode.ON
                    && !mBundle.containsKey(AmbientService.SHUFFLE_MODE)) {
    ...
    private void setRepeatMode() {
        ...
        mRepeatMode = (AmbientService.RepeatMode) mBundle
                .getSerializable(AmbientService.REPEAT_MODE);
    }
    ...
    private void setShuffleMode() {
        ...
        mShuffleState = (AmbientService.ShuffleMode) mBundle
                .getSerializable(AmbientService.SHUFFLE_MODE);
    ...
    /**
     * Helper method used to get the position of the current playing
     * AmbientTrack before and after shuffle
     */
    ...
    private void setPlayPosition() {
        if (mBundle == null
                || !mBundle.containsKey(AmbientService.PLAY_POSITION)) {
            throw new IllegalStateException(AmbientService.TAG
    ...
    /**
     * Alerts the AmbientService about audio focus gain, loss, etc.
     *
     ...
     */
    private void init() {
        sendUpdateBroadcast(AmbientService.PlaybackState.PREPPING_TRACK); // send
        // prepping
        // update
        // to
        // callback
        ...
        try {
            ...
            mPlayer.setDataSource(this, mAmbientTrack.getAudioUri()); // set
            // audio
            // source
    ...
    private void play() {
        try {
            ...
            if (mHandler != null) {
                mHandler.postDelayed(mUpdateProgress,
                        AmbientService.AUDIO_PROGRESS_UPDATE_TIME);
            }

            sendUpdateBroadcast(AmbientService.PlaybackState.PLAY); // sends a
            // now
            // playing
            // update to
            // the
    ...
    private void stop() {
        try {
            ...
            sendUpdateBroadcast(AmbientService.PlaybackState.STOP); // sends a
            // track has
            // stopped
            // update to
            // the
    ...
    private void pause() {
        try {
            ...
            sendUpdateBroadcast(AmbientService.PlaybackState.PAUSE); // sends a
            // track
            // has
            // paused
            // update
    ...
    private void playNext() {
        ...
        if (mPlaylist != null) {
            ...
            if (playPosition >= mPlaylist.size()) {
                sendUpdateBroadcast(AmbientService.PlaybackState.END_OF_PLAYLIST); // send
                // end
                // of
                // playlist
                // update
    ...
    /**
     * Alerts the AmbientService when an AmbientTrack is done playing.
     *
    ...
    public void onCompletion(MediaPlayer mp) {
        ...
        sendUpdateBroadcast(AmbientService.PlaybackState.STOP); // send a stop
        // update to the
        // callback
    ...
    /**
     * Method used to alert the AmbientService that an error has occured with
     * the Media Player
     *
     * @param mp    Media Player object
    ...
    public boolean onError(MediaPlayer mp, int what, int extra) {
        String errorMessage = AmbientService.TAG
        ...
        sendUpdateBroadcast(AmbientService.PlaybackState.ERROR); // send error
        // update to
        // the
        // callback
    ...
    /**
     * Alters the AmbientService when the media player has prepared an
     * AmbientTrack and is ready for play.
     *
     * @param mp media player object
     */
    @Override
    ...
    /**
     * Method used to send an intent to the Ambience Broadcast Receiver to
     * update the callback component.
     *
    ...
    /**
     * Called before the service terminates. All resources used by the
     * AmbientService are cleaned up in this method.
     */
    @Override
     ...
     * information.
     *
     * @param clientPackageName The package name of the application which is requesting access
     *                          to browse media.
     * @param clientUid         The uid of the application which is requesting access to
     *                          browse media.
     * @param rootHints         An optional bundle of service-specific arguments to send to
     *                          the media browse service when connecting and retrieving the
    ...
    /**
     * Called to get the playback state of the media player. This method updates
     * the media session
     */
    ...
    /**
     * Called to get the AmbientTrack bitmap and update the Now Playing Card's
     * meta data on AndroidTV. The AmbientService uses the Picasso Library by
     * Square to get the album image from memory, cache or internet.
     *
    ...
    private void metadataBitmapHelper() {
        ...
        try {
            Picasso.with(this).load(mAmbientTrack.getAlbumImageUri())
                    ...
                    .error(placeholderDrawable).into(new Target() {
                @Override
                public void onBitmapLoaded(Bitmap bitmap,
                                           Picasso.LoadedFrom loadedFrom) {

                    if (bitmap == null) {
                        bitmap = ((BitmapDrawable) placeholderDrawable)
                                .getBitmap();
                    }
                ...
                public void onBitmapFailed(Drawable drawable) {
                    ...
                    updateMetadata(((BitmapDrawable) drawable)
                            .getBitmap());
                }
    ...
    /**
     * Method used to update the meta data of the Now Playing Card with data
     * from the current playing Ambient Track
     *
    ...
    private void updateMetadata(Bitmap bitmap) {
        ...
        metadataBuilder.putString(MediaMetadata.METADATA_KEY_DISPLAY_TITLE,
                mAmbientTrack.getName());
        metadataBuilder.putString(MediaMetadata.METADATA_KEY_DISPLAY_SUBTITLE,
                mAmbientTrack.getAlbumName());

        metadataBuilder.putString(MediaMetadata.METADATA_KEY_DISPLAY_ICON_URI,
                mAmbientTrack.getAlbumImageUri().toString());

        // And at minimum the title and artist for legacy support
        metadataBuilder.putString(MediaMetadata.METADATA_KEY_TITLE,
                mAmbientTrack.getName());
        metadataBuilder.putString(MediaMetadata.METADATA_KEY_ARTIST,
                mAmbientTrack.getArtistName());
     ...
     * and the system. A callback may be set using
    ...
    private class MediaSessionCallback extends MediaSession.Callback {
        ...
        /**
         * Called when a controller has sent a command to this session. The
         * owner of the session may handle custom commands but is not required
         * to.
         *
         * @param command The command name.
         * @param args    Optional parameters for the command, may be null.
         * @param cb      A result receiver to which a result may be sent by the
         *                command, may be null.
         */
        @Override
                ...
                if (ke != null && ke.getAction() == KeyEvent.ACTION_DOWN) {
                    ...
                    switch (ke.getKeyCode()) {
                        ...
                        case KeyEvent.KEYCODE_HEADSETHOOK:
                            boolean isPlaying = state == null ? false : state
        ...
        /**
         * Called when a MediaController wants a PlaybackState.CustomAction to
         * be performed.
         *
