 ...
 * for audio. Ambient Service is advanced and has all basic audio control types
 * like play,pause,stop,resume,previous,skip, shuffle and repeat. The Service
 * also creates A notification in the Notification Drawer to control playback.
 ...
 * @version 1.3
 */
public class AmbientService extends Service implements
     ...
     */
    public final static String TAG = "com.radioayah.ambience.AmbientService";

    /**
     * Tag used to identify the AmbientService incoming broadcast receiver
     */
    public final static String AMBIENT_SERVICE_BROADCASTER = TAG
            + ".INCOMING_REQUEST_BROADCASTER";
    ...
    /**
     * Tag used to identify a track that needs to be removed from the the
     * AmbientService playlist
     */
    ...
    private final Runnable mUpdateProgress = new Runnable() {
        ...
        public void run() {
            ...
            if (mHandler != null && mPlayer != null && mPlayer.isPlaying()) {
                ...
                bundle.putSerializable(PLAYBACK_STATE,
                        PlaybackState.CURRENT_PLAYING_TRACK_INFO);
    ...
    /**
     * Holds the intent filter action name used to launch an activity when a
     * notification is tapped in the navigation drawer.
     */
    ...
    private final BroadcastReceiver IncomingRequestReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            if (intent == null || intent.getExtras() == null) {
                Log.e(TAG,
                        "Passed an empty intent to IncomingRequestBroadcaster");
                return;
            }
            ...
            if (mBundle.containsKey(PLAYBACK_STATE)) {
                ...
                try {
                    PlaybackState state = (PlaybackState) mBundle
                            .getSerializable(PLAYBACK_STATE);
    ...
    private void createPlaylist() {
        ...
        if (newTracks.size() > 0) { // replace existing playlist
            mPlaylist = new ArrayList<AmbientTrack>();
        }
    ...
    private void removeTrackFromPlaylist() {
        if (mBundle.getParcelable(REMOVE_TRACK) != null
                && mOriginalPlaylist != null && mPlaylist != null) {
            mOriginalPlaylist.remove(mBundle.getParcelable(REMOVE_TRACK));
            mPlaylist.remove(mBundle.getParcelable(REMOVE_TRACK));
        }
    }
     ...
     */
    private void init() {
        sendUpdateBroadcast(PlaybackState.PREPPING_TRACK); // send prepping
        // update to
        // callback
        ...
        try {
            ...
            if (mPlaylist == null || mPlaylist.get(playPosition) == null) {
                Log.e(TAG,
                        ": The AmbientTrack item was null. Check the quality of your playlist before"
            ...
            mPlayer.setDataSource(mAmbientTrack.getAudioUri().toString()); // set
    ...
    private void play() {
        try {
            ...
            sendUpdateBroadcast(PlaybackState.PLAY); // sends a now playing
            // update to the
            // callback
            createNotification();
        } catch (Exception e) {
    ...
    private void stop() {
        try {
            ...
            sendUpdateBroadcast(PlaybackState.STOP); // sends a track has
            // stopped update to the
            // callback
        } catch (Exception e) {
            e.printStackTrace();
            Log.e(TAG, e.getMessage());
    ...
    private void pause() {
        try {
            ...
            sendUpdateBroadcast(PlaybackState.PAUSE); // sends a track has
            // paused update to the
            // callback
            createNotification();
        } catch (Exception e) {
            e.printStackTrace();
    ...
    private void playNext() {
        ...
        if (mPlaylist != null) {
            ...
            if (playPosition >= mPlaylist.size()) {
                sendUpdateBroadcast(PlaybackState.END_OF_PLAYLIST); // send end
                // of
                // playlist
                // update to
                // callback
    ...
    public void onCompletion(MediaPlayer mp) {
        ...
        sendUpdateBroadcast(PlaybackState.STOP); // send a stop update to the
        // callback

        if (mNotificationManager != null) {
            mNotificationManager.cancel(NOTIFICATION_CONTROL_ID);
        }
        if (mOriginalPlaylist.size() > 1) {
    ...
    /**
     * Method used to alert the AmbientService that an error has occurred with
     * the Media Player
     *
     * @param mp    Media Player object
    ...
    public boolean onError(MediaPlayer mp, int what, int extra) {
        ...
        sendUpdateBroadcast(PlaybackState.ERROR); // send error update to the
        // callback
        if (mNotificationManager != null) {
            mNotificationManager.cancel(NOTIFICATION_CONTROL_ID);
        }
     ...
     * <p/>
     * A bundle object is appended to the notification pendingIntent. This
     * bundle contains the ambientService current playing track, playlist,
     * shuffle mode, repeat mode, volume level and action launcher string. A
     * user app can use this information to restore state.
     * <p/>
     * The AmbientService uses the Picasso Library by Square to get the album
     * image from memory, cache or internet.
     *
        ...
        try {
            ...
            Picasso.with(this).load(mAmbientTrack.getAlbumImageUri())
                ...
                public void onBitmapFailed(Drawable drawable) {
                    ...
                    buildNotification(((BitmapDrawable) drawable)
                            .getBitmap());
                }
            ...
            buildNotification(((BitmapDrawable) placeholderDrawable)
                    .getBitmap());
        }
    ...
    /**
     * Helper method used to build a notification. Notifications are compatible
    ...
    private void buildNotification(Bitmap bitmap) {
        ...
        NotificationCompat.Builder builder = new NotificationCompat.Builder(
                this).setSmallIcon(R.drawable.ic_launcher); // replace with app
        // logo by naming
        // your app logo
        // app_icon
        ...
        if (mActivityLauncher != null) {
            ...
            activityIntent.putExtras(buildAmbientServiceBundle()); // pass
            // ambient
            // service
            // state

            PendingIntent activityPending = PendingIntent.getActivity(
                    AmbientService.this, 800, activityIntent,
                    PendingIntent.FLAG_UPDATE_CURRENT);

            builder.setContentIntent(activityPending);
        }

        if (mAmbientTrack.getName() != null
                && !mAmbientTrack.getName().equals("")) {
        ...
        if (mAmbientTrack.getAlbumName() != null
                && !mAmbientTrack.getAlbumName().equals("")) {
        ...
        PendingIntent PrevIntent = PendingIntent.getBroadcast(
                AmbientService.this, 801, previousIntent,
                PendingIntent.FLAG_UPDATE_CURRENT);
        ...
        PendingIntent pIntent = PendingIntent.getBroadcast(AmbientService.this,
                802, playIntent, PendingIntent.FLAG_UPDATE_CURRENT);
        ...
        PendingIntent NextIntent = PendingIntent.getBroadcast(
                AmbientService.this, 803, forwardIntent,
                PendingIntent.FLAG_UPDATE_CURRENT);
    ...
    public enum RepeatMode {
        REPEAT_ONE, REPEAT_ALL, ShuffleMode, RepeatMode, OFF
    }
    ...
    public enum PlaybackState {
        PLAY, STOP, PAUSE, RESUME, SKIP, PREVIOUS, PREPPING_TRACK, END_OF_PLAYLIST, CURRENT_PLAYING_TRACK_INFO, ERROR, SERVICE_STARTED, SERVICE_STOPPED
    }
